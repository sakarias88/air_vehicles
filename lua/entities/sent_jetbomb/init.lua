...
function ENT:PhysicsCollide( data, phys ) 
	...
	if self.ExplodeOnce == 0 then
		...
		FireExp:Fire("kill", "", 5)
		util.BlastDamage( self:GetOwner(), self:GetOwner(), self.Entity:GetPos(), 400, 100)
...
function ENT:PhysicsUpdate( physics )
	...
	local AimVec = self.Entity:GetVelocity():GetNormalized():Angle()					
...
function ENT:OnTakeDamage(dmg)
	...
	if self.ExplosionDel < CurTime() and self.ExplodeOnce == 0 && not(dmg:IsExplosionDamage()) then
		...
		local nor = self.Entity:GetVelocity():GetNormalized()
