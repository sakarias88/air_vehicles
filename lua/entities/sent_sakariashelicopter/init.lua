...
ENT.FireMissileDelay = 5
ENT.FireMissilesCurDelay = 0
ENT.FireMissiles = false
ENT.MissileFireDelay = 0.2
ENT.MissileFireCurDelay = 0
ENT.NrOfMissilesToFire = 4
ENT.CurNrOfMissilesToFire = 0

ENT.TurretFire = false
ENT.LaserPos = Vector(0,0,0)
ENT.GuidedMissile = NULL
ENT.GuidedDelay = 2
ENT.GuidedCurDelay = 0
ENT.IsGuiding = false

ENT.WepSwitchDel = 0

--Weapons
ENT.TurretDel = CurTime()
ENT.PlyAim = Vector(0,0,0)
ENT.wepName = {"Search Light", "Turret", "Missiles", "Guided Missile"}
ENT.Seats = {}
ENT.SeatPos = {
Vector(28,-14.5,-14),
Vector(28,14.5,-14),
...
function ENT:Use( activator, caller )
    if self.UseDelay < CurTime() and self.Destroyed == false && activator:IsPlayer() && (activator.LstEntr == nil or activator.LstEntr < CurTime()) && self.Entity:WaterLevel() <= 1 then
		self.UseDelay = CurTime() + 1
        for k,v in ipairs(self.Seats) do
            local driver = v:GetDriver()
            if !IsValid(driver) then
                activator:EnterVehicle( v )
...
if self.UpdatePhys > RealTime() then
    if self.Destroyed == false && self.TopRotorConstraint != NULL then
        ...
        if self:GetDriver() != nil then
            ...
            local driver = self:GetDriver()
            --Weapons
            if self.WepSwitchDel < CurTime() and driver:KeyDown( IN_SPEED ) then
				self.WepSwitchDel = CurTime() + 0.5
                self.CurWep = self.CurWep + 1
                self:EmitSound("buttons/blip1.wav")
                self.Sounds.ShootSound:Stop()
                
                if self.CurWep > 4 then
                    self.CurWep = 1
                end
                driver:PrintMessage( HUD_PRINTTALK, self.wepName[self.CurWep])
            end
            ...
            --Update turret
            if self.CurWep == 2 then
                self:UpdateLaser(driver, true)
			elseif self.CurWep == 4 then
				self:UpdateLaser(driver, false)
            else
            ...
            if driver:KeyDown( IN_ATTACK ) and self.FireState == 0 then
				self.FireState = 1
			elseif self.FireState == 1 then
				self.FireState = 2
			elseif !driver:KeyDown( IN_ATTACK ) then
				self.FireState = 0
            end    	

           --if driver:KeyReleased( IN_ATTACK ) then
                --self.FireState = 0
           -- end   

	        if self.CurWep == 1 and self.FireState == 1 then
				if self.LampState then
					self:LampOff()
				else
					self:LampOn()
				end
			elseif self.CurWep == 2 then
				if self.FireState == 2 then
					self.TurretFire = true
					if self.TurretDel < CurTime() then
						self.TurretDel = CurTime() + 0.06
						self.Sounds.ShootSound:Stop()
						self.Sounds.ShootSound:Play()
						
						self:Shoot(1,
						self:GetPos() + ( self:GetForward() * 60 ) + ( self:GetUp() * -30 ),
						self.PlyAim,
						Vector(0.03,0.03,0),
						driver)
					end			
				elseif self.TurretFire == true then
					self.TurretFire = false
					self.Sounds.ShootSound:Stop()
					self.Sounds.StopShootSound:Stop()
					self.Sounds.StopShootSound:Play()
				end
			elseif self.CurWep == 3 and self.FireState == 1 and self.FireMissilesCurDelay < CurTime()  then
				self.FireMissilesCurDelay = CurTime() + self.FireMissileDelay
				self.FireMissiles = true
				self.CurNrOfMissilesToFire = true
				self.CurNrOfMissilesToFire = self.NrOfMissilesToFire
			elseif self.CurWep == 4 and self.FireState == 1 and self.IsGuiding == false and self.GuidedCurDelay < CurTime() then

				self.IsGuiding = true
				self.GuidedMissile = ents.Create( "sent_GuidedHeliMissile" )     
				self.GuidedMissile:SetPos( self:GetPos() + self:GetUp() * -50)    
				self.GuidedMissile:SetAngles( self:GetAngles() )
				self.GuidedMissile:Spawn()
				self.GuidedMissile:SetOwner(driver)
            end		
	
			if self.IsGuiding == true then
				if IsValid(self.GuidedMissile) then
					self.GuidedMissile:SetLaserPos( self.LaserPos )
				else
					self:KillGuidedMissile()
				end
			end

			
			
			if self.FireMissiles == true and self.MissileFireCurDelay < CurTime() then
				self.MissileFireCurDelay = CurTime() + self.MissileFireDelay
				self.CurNrOfMissilesToFire = self.CurNrOfMissilesToFire - 1
				
				local missile = ents.Create( "sent_NoTargetJetMissile" )     
				missile:SetPos( self:GetPos() + self:GetUp() * -50)    
				missile:SetAngles( self:GetAngles() )
				missile:Spawn()
				missile:SetOwner(driver)
				
				if self.CurNrOfMissilesToFire <= 0 then
					self.FireMissiles = false
				end
			end   
...
function ENT:HpAdjustments()
    ...
    if self.IsOn == true && IsValid(self.TopRotor) && IsValid( self.TopRotorConstraint ) then
        ...
        if SpeedD > -1500 and SpeedD < -300 then
            ...
            self.TopRotorHealth = self.TopRotorHealth - 20
        end    
    end

    if IsValid(self.TopRotor) && self.TopRotorConstraint != NULL && self.TopRotorHealth < 60 then
    ...
    if self.TopRotorHealth <= 0 && IsValid(self.TopRotor) && self.TopRotorConstraint != NULL then
        self:RemoveTopRotor()
    end        
    ...
    if self.Destroyed == false && self.HP <= 0 then
        self.Destroyed = true
        self.HpStage = 0
...
function ENT:GetNextEmptySeat( start )
	...
	for i = start, (4 + start) do
		local slot = i % (4 + 1)
		if slot != 0 && IsValid( self.Seats[slot] ) then
			return slot
		end
	end
        ...
        if IsValid(v) then
            ...
            if IsValid(driver) then
				...
				if driver:KeyDown( IN_ATTACK2 ) and driver.SwapDel < CurTime() then
					...
					local seatNum = self:GetNextEmptySeat( k+1 )
